...
final class GameViewModel {
    ...
    func openSession(now: Date = Date()) async {
        ...
        let progress = AsyncStream<(Int, Int)> { continuation in
            Task.detached(priority: .userInitiated) {
                // **Time the slices and size the next one by what the last one
                // cost.** Head-count was a proxy for that cost and it is not a
                // good enough one: it is fixed at the start of an absence the
                // colony grows all the way through, and at a hundred and sixty
                // colonists the first slice still ran for seconds with the bar
                // reading zero. A clock outside the engine can simply measure.
                var last = Date()
                let caught = GameEngine.openSession(
                    snapshot, now: now, registry: registry, sliceTicks: slice,
                    stoppingWhen: halt,
                    nextSlice: { ran in
                        let took = Date().timeIntervalSince(last)
                        last = Date()
                        return Self.resized(ran, took: took)
                    },
    ...
    /// **What the next slice should be**, given what the last one ran and how
    /// long it took.
    ///
    /// The bar is only a bar if it moves: a slice wants to land inside a couple
    /// of frames, not inside a couple of seconds. Aim at `sliceSeconds`, correct
    /// by the ratio, and clamp hard on both sides — a colony too heavy to manage
    /// even sixteen ticks still makes progress, and a small one does not pay
    /// per-slice overhead a thousand times over.
    ///
    /// Halved and doubled at most, so a single slow slice (the app went to the
    /// background, the phone throttled) does not send the size to either end.
    nonisolated static func resized(_ ran: Int, took: TimeInterval) -> Int {
        guard took > 0 else { return min(maxSlice, ran * 2) }
        let ratio = min(2, max(0.5, sliceSeconds / took))
        return max(minSlice, min(maxSlice, Int(Double(ran) * ratio)))
    }

    /// How long one slice should take. Two frames at 60 Hz, near enough: short
    /// enough that the count is visibly climbing, long enough that the work
    /// between reports is worth more than the reporting.
    nonisolated static let sliceSeconds: TimeInterval = 0.033
    nonisolated static let minSlice = 4
    nonisolated static let maxSlice = 2000
    ...
    /// The fight that has just stopped, and when — real seconds, because the
    /// world clock is paused for a raid and cannot measure the seconds after
    /// one. See `advanceSiegeStep`.
    var siegeEnded: (id: UUID, at: Date)?
    ...
    func advanceSiegeStep(now: Date = Date()) -> Bool {
        ...
        guard world.settlements.contains(where: { $0.siege != nil }) else {
            siegeStepLandedAt = nil
            // **When the fighting stopped, in the canvas's own timebase.**
            //
            // The field is supposed to linger for a few seconds after the last
            // blow — the dead lie where they fell, the blood stays, the torches
            // gutter. That linger was measured off `continuousTick`, and a raid
            // **pauses the world**: the clock is frozen a tick past the one the
            // fight started in, so the moment the siege ended the elapsed share
            // came out at nine play-throughs and everything vanished between
            // one frame and the next. Rule 103 a third time, and the same cure:
            // stamp it against the clock the thing is actually happening on.
            if let finished = concluded.last {
                siegeEnded = (id: finished.id, at: now)
            }
    ...
    /// What the player has typed into the bench's search field, and whether the
    /// list is folded to what the colony can make right now.
    ///
    /// **Stored, so they stay here.** The rest of the bench's list is a pure
    /// derivation and lives in `GameViewModel+Crafting.swift`; an extension
    /// cannot hold stored properties, and these two are the panel's own memory
    /// — a search box that clears itself every time the world ticks is worse
    /// than no search box.
